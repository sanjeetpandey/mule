...
abstract class AbstractModelLoaderDelegate {
  ...
  private Class<?> resolveConnectionType(ExecutableComponentDeclarer componentDeclarer, ExtensionParameter connectionParameter,
                                         WithAlias alias) {
    org.mule.runtime.module.extension.internal.loader.java.type.Type theRealType = connectionParameter.getType();


    if (theRealType.getTypeName().startsWith(ConnectionProvider.class.getName())) {
      List<GenericInfo> generics = theRealType.getGenerics();
      if (generics.size() == 0) {
        throw new IllegalOperationModelDefinitionException(format(
    ...
    return theRealType.getDeclaringClass();
  }
